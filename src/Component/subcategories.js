...
export default function Subcategory() {
    ...
    console.log(code + "------------------------------------------")
    useEffect(() => {
        fetch("http://localhost:8080/categories/api/subcategories/" + code).then(res => res.json())
            .then(result => {
                setCategory(result);
console.log(result)
            }
                ...
                <div className="mx-auto max-w-2xl py-10 sm:py-20 lg:max-w-none lg:py-8">
                    <div className=" flex flex-wrap justify-start -mx-2 items-start ">
                        {Category.map((product) => (
                            <div key={product.categoryId} className="group relative px-4">
                                <div className="relative h-80 w-full overflow-hidden rounded-lg bg-white sm:aspect-h-1 sm:aspect-w-2 lg:aspect-h-1 lg:aspect-w-1 group-hover:opacity-75 sm:h-64">
                                    <img
                                        src={product.categoryImgPath}
                                        alt={product.categoryName}
                                        className="h-full w-full object-cover object-center"
                                    />
                                </div>
                                <h3 className="mt-6 text-sm text-gray-500" onClick={()=>handlesubcategory(product.categoryId) }>
                                    <a href={product.href}>
